...
//    }
//
//    // 如果x!=null且x的Class==kc,则返回k.compareTo(x)
...
//            	// 如果数组长度大于最大值(1 << 30) ——> 直接返回旧的数组，不再进行扩容
//                threshold = Integer.MAX_VALUE;
//                return oldTab;
//            }
//            else if ((newCap = oldCap << 1) < MAXIMUM_CAPACITY &&
//                     oldCap >= DEFAULT_INITIAL_CAPACITY)
//            	// 当前数组长度大于16，并且扩展一倍后，长度小于最大值  ——> threshold=oldThr*2
...
//        }
//        if (newThr == 0) {
//        	// newThr为0 ——> 根据newCap和loadFactor计算
...
//                	// 将索引为j的hash桶释放掉，以便GC
//                    oldTab[j] = null;
//                    // 如果当前链表中就有一个元素(没有发生哈希碰撞)
//                    if (e.next == null)
//                    	// 直接将这个元素放入新的哈希桶中
//                        newTab[e.hash & (newCap - 1)] = e;
//                    else if (e instanceof TreeNode)
//                    	// 如果哈希桶元素类型时红黑树节点，就进行红黑树相关处理
...
//                tl = p;
//            } while ((e = e.next) != null);
//            // 上面只是将链表转换为双链表，下面让桶的第一个元素指向新建的红黑树头节点，以后这个桶里的元素就是红黑树而不是链表了
...
//         * 之前的操作并没有设置红黑树的颜色值，现在得到的只能算是双向链表。在调用树形节点hd.treeify(tab)方法进行塑造红黑树，这是HashMap中个人认为第二个比较难得方法
//         * 
//         */
